...
Claude Code specific: requires `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1`. In ZCode this opt-in does not exist — treat agent teams as unavailable; use sequential subagent delegation instead.
...
**Current status**: Claude Code only (opt-in env var). Not available in ZCode — use vertical/horizontal delegation instead.
